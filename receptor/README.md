...
4. **Demodulación lógica PAM4 (`decodificar_pam4`)**  
...
## 6. Cómo ejecutar mis dos códigos (ESP32 y PC)

### 6.1. Código de la ESP32

1. **Requisitos**
   - Una placa **ESP32** con **MicroPython** cargado.
   - Herramienta para subir código (por ejemplo **Thonny**, **uPyCraft** o similar).

2. **Preparación**
   - Abrir el archivo `main.py` en el editor.
   - Configurar:
     - `SSID` y `PASS` con el nombre y clave de la red WiFi.
     - `PC_IP` con la **IP de tu PC receptora**.
     - `PC_PORT_TX` con el puerto donde va a escuchar la PC (por defecto 9100).

3. **Carga en la placa**
   - Conectar la ESP32 por USB.
   - En Thonny/uPyCraft, seleccionar el intérprete MicroPython (ESP32).
   - Guardar el archivo como `main.py` en la **memoria de la ESP32**.

4. **Ejecución**
   - Reiniciar la ESP32 (reset o desconectar/conectar).
   - La ESP32:
     - Se conecta a la red WiFi.
     - Escucha al canal en el puerto `5052`.
     - Cuando recibe datos, los agrupa en frames de 68 bytes y los envía a la PC en el puerto configurado.
...
1. **Requisitos**
   ...
   - Estar en la misma red que la ESP32 (misma subred IP).

2. **Preparación**
   - Crear un archivo, por ejemplo `pc_receptor.py`.
   - Pegar el código de la PC dentro de ese archivo.
   - Asegurarse de que:
     - `HOST = "0.0.0.0"` (acepta conexiones en todas las interfaces).
     - `PORT = 9100` coincide con `PC_PORT_TX` de la ESP32.
     - `VIS_IP` y `VIS_PORT` apunten al visualizador que vaya a usar el sistema (si corresponde).

3. **Ejecución**
   - Abrir una terminal o consola.
   - Ir a la carpeta donde está `pc_receptor.py`.
   - Ejecutar:

     ```bash
     python pc_receptor.py
     ```

   - La PC:
     - Espera conexiones entrantes de la ESP32 en el puerto 9100.
     - Cada vez que llega un frame de 68 bytes:
       - Muestra por consola la cabecera (`'hola'`) y los símbolos PAM4 decodificados.
       - Reenvía esos 68 bytes al visualizador.
